...
{{--                                <th>(含预存金额)</th>--}}
                            ...
                            <tr>
                                ...
                                <td>{{str_limit($site->site_name)}}</td>
                                <td>{{str_limit(@$site->company->liaison,25)}}</td>
                                <td>{{str_limit(@$site->company->liaison_phone,25)}}</td>
                                <td>{{@$agent->fund}}</td>
{{--                                <td>待完善</td>--}}
                                <td>{{@$agent->totalprice}}</td>
                                <td>{{@$agent->totalprice-@$agent->fund}}</td>
                            </tr>
                            </tbody>
                        </table>
                </div>
        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header" style="background-color: #D3D4D3">
                    <h3 class="box-title">当前余额:<span style="color: red;">{{@$agent->fund}}</span>元,</h3>
                    &emsp;
                    ...
                    @if($re->count())
                        <table class="table table-bordered table-hover">
                            ...
                            @foreach($re as $k=>$v)
                                <tr>
                                    <td>{{$v->mOrderID}}</td>
                                    <td>{{str_limit(@$v->TPayOrderMain->site3E21->site_name,25)}}_{{str_limit
                                    (@$v->TPayOrderMain->company->name,25)}}[{{@$v->TPayOrderMain->company->id}}]</td>
